- Introduction -
Hi! This is Keisuke (Keith) Oshima. I built this script for fun after noticing that that the class genes in the ABC Model 
...
The script operates on four levels:
	(1) A grid that that prints ABC gene expression as a list in the python console.
		a. This is used to generate the tkinter grid in (4).

	(2) An algorithm that divides the top (B) and bottom (A and C) of the ABC model. 
	    It goes through possible item combinations from (3) and updates the console grid.
		a. Why divide them? Each level has different rules. Divide and conquer.
	...
	(3) A simple GUI with 4 whorl buttons and 3 gene buttons. These inputs disable gene expression. 
		a. Genes are given priority before whorls.
		   This means that pressing A and whorl 1 will disable A only in W1.
		b. Inputs are placed in a list and used in (2).

	(4) A tkinter grid of empty labels that are later filled with a specific floral organ image.
		a. Why a grid and not a single image? 7 buttons == A lot of permutations/combinations...
		   ...
		   Even with the v1.2 where I sorted inputs, there are a lot of combinations to find.   
			- (7 choose 1) + (7 choose 2 + (7 choose 3) + (7 choose 4) 
			  + (7 choose 5) + (7 choose 6) + (7 choose 7) = ~127
		   A grid that auto-generates an image removes the need to check for a combination and
		   add a specific image. 
		   It does come at the cost of neatness. Check the "Whole Images" folder ( ; _ ;)

- In-Progress - (In order of easiest to hardest)
