...
class ABCWindow(tk.Tk):
    ...
    def updating_abc(self):
        ...
        if (ABCWindow.target_gene == [] and ABCWindow.target_whorl == []) or \
                ...
                ABCWindow.target_gene == ['B'] and ABCWindow.target_whorl == ['Carpels']:
        ...
        # Whorl 3 & 4 disabled
        if ABCWindow.target_gene == [] and \
                (ABCWindow.target_whorl == ['Stamen', 'Carpels'] or ABCWindow.target_whorl == ['Carpels', 'Stamens']):
            # top adjustments
            update_cls_b.config(width=12)
            empty_label_bl.grid(row=1, column=3)
            empty_label_b.grid(row=1, column=1)
        ...
        if ABCWindow.target_gene == ['B'] and ABCWindow.target_whorl == ['Petals'] or \
                (ABCWindow.target_gene == ['B', 'C'] or ABCWindow.target_gene == ['C','B']) and ABCWindow.target_whorl == ['Petals']:
            update_cls_b.config(width=12)
            empty_label_bl.grid(row=1, column=1)
            empty_label_b.grid(row=1, column=3)
        ...
        if (ABCWindow.target_gene == ['B'] and ABCWindow.target_whorl == ['Stamen']) or \
                (ABCWindow.target_gene == ['A', 'B'] or ABCWindow.target_gene == ['B',
                                                                                  'A']) and ABCWindow.target_whorl == [
            'Stamen']:
            update_cls_b.config(width=12)
            update_cls_b.grid(row=1, column=2)
            empty_label_bl.grid(row=1, column=3)
        ...
        # A & C disabled
        if (ABCWindow.target_gene == ['C', 'A'] or ABCWindow.target_gene == ['A', 'C']) and ABCWindow.target_whorl == []:
            update_cls_a.destroy()
            update_cls_c.destroy()
            empty_label_acl.grid(row=2, column=1)
        ...
        # # B & C disabled
        # if (ABCWindow.target_gene == ['B', 'C'] or ABCWindow.target_gene == ['C', 'B']) and ABCWindow.target_whorl == []:
        #     update_cls_b.destroy()
        #     update_cls_c.destroy()
        #     empty_label_bl.grid()
        #     update_cls_a.config(width=52)
...
# Robles, Pedro & Pelaz, Soraya. (2005). Flower and fruit development in Arabidopsis thaliana.
# The International journal of developmental biology. 49. 633-43. 10.1387/ijdb.052020pr.
